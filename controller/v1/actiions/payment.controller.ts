...
export default class PaymentsController {
  ...
  static async getAll(req: Request, res: Response): Promise<object> {
    const data: any = await prisma.payments.findMany({
      where: {
        active: true,
      },
    });
  ...
  static async getOne(req: Request, res: Response): Promise<object> {
    ...
    const data: any = await prisma.payments.findUnique({
      where: {
        id,
      },
    });
   ...
   * @returns
   */
  static async getOneUsingValue(req: Request, res: Response): Promise<object> {
    const title: any = req.params.title;

    const data: any = await prisma.payments.findFirst({
      where: {
        title,
        active: true,
      },
  ...
  static async add(req: Request, res: Response): Promise<object> {
    try {
      ...
      // validate body
      const notValide: any = validate(body, Validation.payments());
  ...
  static async delete(req: Request, res: Response): Promise<object> {
    try {
      ...
      // Save data
      const data: any = await prisma.payments.update({
        where: {
          id,
        },
        data: {
      ...
      return okRes(res, {
        errMsg: `The ${data.title} Token has been Deactivated successfully.`,
      });
    } catch {
      return errRes(res, messageReturn("wentWrong"));
    }
  ...
  static async update(req: Request, res: Response): Promise<object> {
    try {
      ...
      const fieldsToUpdate = ["title", "description", "detail1", "detail2"];
      ...
      // Save data
      const dataUpdated: any = await prisma.payments.update({
        where: {
          id,
        },
        data: data,
      ...
      return okRes(
        res,
        `The ${dataUpdated.title} Token has been activated successfully.`
      );
    } catch {
      return errRes(res, messageReturn("wentWrong"));
    }
